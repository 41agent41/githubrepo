...
**Reading order:** 3 of 4 — Read after [1-OPERATIONS.md](1-OPERATIONS.md). Next: [3-FEATURES.md](3-FEATURES.md)
...
1. **Setup SSH Config** — Add hosts to `~/.ssh/config` (Windows: `C:\Users\seana\.ssh\config`). Copy from `ssh-config-template` or paste the config below.
2. **Test SSH** — Run `ssh tradingapp`. If prompted for password, run `ssh-keygen -t ed25519` and `ssh-copy-id tradingapp@10.7.3.20`.
3. **Connect Cursor** — `F1` or `Ctrl+Shift+P` → "Remote-SSH: Connect to Host" → select `tradingapp` → open folder `/home/tradingapp/tradingapp`.

You can then edit files directly on the server, use the terminal (`Ctrl+` `), and run `docker-compose restart backend` or `./tradingapp.sh logs` as needed.

---

## Quick Setup - Cursor Remote SSH (Detailed)
